...
TEST_CASE("Polygon collision with zero vertices") {
  ...
  REQUIRE_FALSE(checkPolygonCollision(empty_polygon, quad));
  REQUIRE_FALSE(checkPolygonCollision(quad, empty_polygon));
}
...
TEST_CASE("Polygon collision check with point (one vertex)") {
  ...
  SUBCASE("No collision") {
    ...
    REQUIRE_FALSE(checkPolygonCollision(point, triangle));
    REQUIRE_FALSE(checkPolygonCollision(triangle, point));
  }

  SUBCASE("Collision") {
    const std::array point{glm::vec2{-0.5, 0}};
    REQUIRE(checkPolygonCollision(point, triangle));
    REQUIRE(checkPolygonCollision(triangle, point));
  }
}
...
TEST_CASE("Polygon collision resolving with point (one vertex)") {
  SUBCASE("Move point out of rectangle") {
    const std::array point{glm::vec2{-0.5, 0}};
    const auto displacement = checkPolygonCollision(point, quad);
  ...
  SUBCASE("Move rectangle out of point") {
    const std::array point{glm::vec2{-0.5, 0}};
    const auto displacement = checkPolygonCollision(quad, point);
    REQUIRE(displacement);
    REQUIRE(displacement->x == doctest::Approx(0.5));
    REQUIRE(displacement->y == doctest::Approx(0));
  }
...
TEST_CASE("Polygon collision resolving with line") {
  ...
  SUBCASE("No collision") {
    ...
    REQUIRE_FALSE(checkPolygonCollision(triangle, line));
    REQUIRE_FALSE(checkPolygonCollision(line, triangle));
  }

  SUBCASE("Move line out of triangle") {
    const std::array line{glm::vec2{-0.4, -0.5}, glm::vec2{-0.9, -1.9}};
    const auto displacement = checkPolygonCollision(line, triangle);
    REQUIRE(displacement);
    REQUIRE(displacement->x == doctest::Approx(0));
    REQUIRE(displacement->y == doctest::Approx(-0.5));
  }
  ...
  SUBCASE("Move triangle out of line") {
    ...
    const auto displacement = checkPolygonCollision(triangle, line);
    REQUIRE(displacement);
...
TEST_CASE("Polygon collision resolving is independent of edge orientation") {
  SUBCASE("Clockwise triangle") {
    ...
    const auto displacement = checkPolygonCollision(triangle, quad);
    REQUIRE(displacement);
...
TEST_CASE("Polygon collides with itself") { REQUIRE(checkPolygonCollision(quad, quad)); }
...
TEST_CASE("Nested Polygon collision") {
  ...
  const auto displacement = checkPolygonCollision(rectangle, quad);
  REQUIRE(displacement);
...
TEST_CASE("Collision check stops on first failed overlap check") {
  ...
  REQUIRE_FALSE(checkPolygonCollision(line, point));
  REQUIRE_FALSE(checkPolygonCollision(point, line));
}
